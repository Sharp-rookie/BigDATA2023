...
class WallHitPenaltyWrapper(gym.Wrapper):
    ...
    def step(self, action):
        ...
        move = x_position - self.last_x_position
        if move == 0 and action == 1: # 向右走且没有移动
            reward -= 1
...
class JumpRewardWrapper(gym.Wrapper):
    ...
    def __init__(self, env: gym.Env):
        super().__init__(env)
        self.last_y_position = 79

    def step(self, action):
        obs, reward, done, info = self.env.step(action)
        y_position = info['y_pos']
        reward += int(y_position - self.last_y_position > 0) * 1
...
class ScoreRewardWrapper(gym.Wrapper):
    ...
    def step(self, action):
        obs, reward, done, info = self.env.step(action)
        score = info['score']
        diff = score - self.last_score
        if diff == 100: # 踩蘑菇 or 乌龟
            reward += 3
        elif diff == 200: # 金币 or 双杀
            reward += 5
        elif diff == 1000: # 吃蘑菇
            reward += 10
        elif diff == 500: # 踩龟壳
            reward += 5
        self.last_y_position = score
        return obs, reward, done, info
